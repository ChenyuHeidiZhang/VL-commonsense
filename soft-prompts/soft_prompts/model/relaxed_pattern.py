...
class PatternModel:
    ...
    def conditional_generate_single_slot(
        ...
        # list of answers y for all instances in this relation_type
        answers = torch.tensor(util.tokenizer.convert_tokens_to_ids([rel_ins.entities[1] for rel_ins in rel_bank]))
        alternatives = None
        if rel_bank.valence == 3:
            alternatives = torch.tensor(util.tokenizer.convert_tokens_to_ids([rel_ins.entities[2] for rel_ins in rel_bank]))
        # print('answer size', answers.size()) -- [5]
