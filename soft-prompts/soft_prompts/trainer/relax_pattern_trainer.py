...
class RPTrainer:
    ...
    def experiment(
        ...
        for rel_type, pb in pattern_db.items():
            #if rel_type not in ['color', 'shape', 'material', 'cooccur']: continue  # 'coda'
            since = time.time()
            n_cur += 1
            ...
            if not all([len(one_db) > 0 for one_db in splits]):
                # Multi-word obj not acceptable? -- single_lexicon config
        ...
        for state in ['before', 'after']:
            ...
            logger.info(f'Choice Accuracy: {(answer_ranks < alt_ranks).sum() / answer_ranks.shape[0]}')
